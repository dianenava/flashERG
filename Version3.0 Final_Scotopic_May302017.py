...
allfiles = sorted(glob('*normal*.txt'))
listoffiles = []
...
if eye == "L":
    ...
    for x in allfiles:
        ...
        plt.savefig(x + '_OG_SCOTOPIC_.png', format='png')
    ...
    for x in allfiles:
        ...
        # save each file as a text file and as a figure
        # after all the columns have processed, write one big output file and save the figure
        plt.savefig(x + '_OD_SCOTOPIC.png', format='png')
        biggestarray1.append((x, biggerarray1))
        # biggestarray2.append((x, biggerarray2))
...
with open("Final_Scotopic_peaks.txt", mode='a') as file:
    file.write('\n'.join([str(x) for x in output2]))
